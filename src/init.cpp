...
void SetupServerArgs(ArgsManager& argsman, bool can_listen_ipc)
{
    ...
    argsman.AddArg("-matmulrctipverifyjobs=<n>", strprintf("Maximum requested/paid authenticated-tip-child RC ExactReplay jobs per minute while catching up (default: %u, maximum: %u, 0 disables the accelerated lane). Replay remains single-flight; unsolicited and competing-branch work retains the ordinary strict budget.", DEFAULT_MATMUL_RC_TIP_VERIFY_JOBS_PER_MINUTE, MAX_MATMUL_RC_TIP_VERIFY_JOBS_PER_MINUTE), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
