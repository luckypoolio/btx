...
BOOST_AUTO_TEST_CASE(catchup_direct_fetch_requests_only_lowest_hole)
{
    ...
    BOOST_REQUIRE_EQUAL(after_headers.vHeightInFlight.size(), 1U);
    BOOST_CHECK_EQUAL(after_headers.vHeightInFlight.front(), tip->nHeight + 1);
...
struct PeermanBlockingVerify {
    ...
    {
        std::unique_lock<std::mutex> lock(mutex);
        cv.wait(lock, [this] { return running.load() == 0; });
    }
...
BOOST_AUTO_TEST_CASE(handoff_peer_budget_miss_retains_tip_child_body)
{
    ...
    ClearHandoffOverride clear_handoff_override{peerman, gate};
...
BOOST_AUTO_TEST_CASE(global_rc_budget_exhaustion_does_not_disconnect_honest_peer)
{
    ...
    ClearGlobalOverride clear_global_override{peerman, gate};
