# Mainnet assumeutxo refresh — height 190'507

Generated 2026-08-16 from a synced canonical mainnet node (`prune=0`) via
`dumptxoutset type=latest` at height **190507**. File version remains **v9**
...
The previous public pin was 190'467. This pin is the live signed-frontier tip
after the attested-chain lock roll (`d686e143`). Strict consensus nodes
...
The binary `snapshot.dat` (452,998,612 bytes, SHA256 in `SHA256SUMS`) is not
...
```bash
curl -L -o snapshot.dat https://github.com/btxchain/btx/releases/download/assumeutxo-190507/snapshot.dat
