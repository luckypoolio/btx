...
class ConfArgsTest(BitcoinTestFramework):
    ...
    def test_connect_with_seednode(self):
        ...
        # A double-negative enable (-noconnect=0) must not leak a synthetic
        # "connect=1" target (which would parse as 0.0.0.1).
        with self.nodes[0].assert_debug_log(
                expected_msgs=seednode_ignored,
                unexpected_msgs=['0.0.0.1:']):
            self.restart_node(0, extra_args=['-connect=fakeaddress1', '-noconnect=0', '-seednode=fakeaddress2', UNREACHABLE_PROXY_ARG])
